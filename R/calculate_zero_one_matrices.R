...
calculate_zero_one_matrices <- function( model, intervention_names, outcome_names, verbose ){
  ...
  if( is.character( intervention_names ) && all( intervention_names %in% model$info_model$var_names ) ){
    ...
    intervention_labels <- model$info_interventions$intervention_name
  }

  # get variable names of outcome variables
  if( is.character( outcome_names ) && outcome_names %in% setdiff(model$info_model$var_names, intervention_labels) ){
  ...
  for (i in 1:n_int){
    intervention_index[i] <- which(intervention_labels[i] == model$info_model$var_names)
  }
  ...
  select_non_intervention <- matrix(0, nrow = model$info_model$n_ov, ncol = (model$info_model$n_ov - n_int))
  ...
  elimination_matrix <- matrixcalc::elimination.matrix(n = model$info_model$n_ov)
  duplication_matrix <- lavaan::lav_matrix_duplication(n = model$info_model$n_ov)
  commutation_matrix <- lavaan::lav_matrix_commutation(m = model$info_model$n_ov,
